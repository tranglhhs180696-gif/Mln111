...
<body x-data="mainApp()" x-init="init()" :class="darkMode ? 'dark bg-bg text-text' : 'bg-philosophy-bg text-slate-900'"
  ...
  <a href="#main" class="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:bg-white/90 focus:text-black p-2 rounded">Bỏ qua tới nội dung</a>
  ...
  <div x-show="lightbox" x-cloak
    ...
    <button @click.stop="prevInfo()" aria-label="Previous" class="absolute left-6 top-1/2 -translate-y-1/2 z-50 p-3 rounded-full bg-white/10 text-white">‹</button>
    ...
    <button @click.stop="nextInfo()" aria-label="Next" class="absolute right-6 top-1/2 -translate-y-1/2 z-50 p-3 rounded-full bg-white/10 text-white">›</button>
  ...
  <nav role="navigation" aria-label="Main navigation"
    ...
    <div class="hidden md:flex gap-8 text-[10px] font-bold tracking-widest uppercase opacity-70">
      <a href="#hero" :class="activeSection === 'hero' ? 'text-primary' : ''" class="hover:text-primary transition focus-visible:ring-2 focus-visible:ring-primary rounded">Trang chủ</a>
      <a href="#knowledge" :class="activeSection === 'knowledge' ? 'text-primary' : ''" class="hover:text-primary transition focus-visible:ring-2 focus-visible:ring-primary rounded">Học thuật</a>
    ...
    <header id="hero" class="min-h-screen flex items-center justify-center relative pt-16">
      ...
      <div class="container mx-auto px-6 text-center z-10">
        ...
        <div class="flex flex-wrap gap-4 justify-center" data-aos="fade-up" data-aos-delay="200">
          <a href="#knowledge" class="px-8 sm:px-10 py-3 sm:py-4 bg-primary text-white font-bold rounded-xl hover:shadow-lg hover:shadow-primary/30 transition focus-visible:ring-2 focus-visible:ring-primary">TẬP SAN</a>
      ...
      <div class="flex justify-between items-end mb-16 border-b border-gray-200 dark:border-gray-800 pb-8">
        ...
        <div class="flex gap-2">
          <button type="button" @click="contentMode = 'academic'"
                  ...
                  :aria-pressed="contentMode === 'academic'">Hàn lâm</button>
        ...
        <div class="lg:col-span-9 bg-white dark:bg-surface p-8 md:p-12 rounded-3xl shadow-xl transition-all duration-500" data-aos="fade-left">
          <div x-show="contentMode === 'academic'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            <div class="flex items-start justify-between gap-4">
              ...
              <button type="button" @click="speakChapter(activeChapter)" class="text-[12px] px-3 py-2 h-fit rounded bg-primary/10 text-primary" aria-label="Nghe chương">Nghe</button>
            </div>
                ...
                <template x-if="activeChapter === 0">
                  <div>
                    <img src="https://images.unsplash.com/photo-1532012197267-da84d127e765?q=80&w=1200&auto=format&fit=crop"
                         alt="Nguồn gốc nhận thức và xã hội của Triết học" loading="lazy"
                ...
                <template x-if="activeChapter === 1">
                  <div>
                    <img src="https://images.unsplash.com/photo-1507413245164-6160d8298b31?q=80&w=1200&auto=format&fit=crop"
                         alt="Vật chất và Ý thức" loading="lazy"
                         class="w-full h-64 object-cover">
                ...
                <template x-if="activeChapter === 2">
                  <div>
                    <img src="https://images.unsplash.com/photo-1635070041078-e363dbe005cb?q=80&w=1200&auto=format&fit=crop"
                         alt="Quy luật Lượng - Chất" loading="lazy"
                         class="w-full h-64 object-cover">
                ...
                <template x-if="activeChapter === 4">
                  <div>
                    <img src="https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1200&auto=format&fit=crop"
                         alt="Cơ sở hạ tầng" loading="lazy"
                         class="w-full h-64 object-cover">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside class="bg-philosophy-primary text-white p-8 rounded-3xl shadow-xl flex flex-col justify-between" aria-label="Podcast panel">
            <div>
              ...
              <h3 class="text-2xl sm:text-3xl font-serif mt-2 mb-6">Triết học & Deadline</h3>
    ...
    <section id="quiz" class="py-24 container mx-auto px-6 max-w-3xl" x-data="quizApp()">
      <div class="bg-white dark:bg-surface p-8 sm:p-10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl text-center border border-gray-200 dark:border-white/5 transition-all duration-500">
        ...
        <div x-show="!started && !finished">
          <p class="mb-6 sm:mb-10 opacity-70">Bạn là "Nhà biện chứng" hay "Tấm chiếu mới"?</p>
          <button type="button" @click="start()" class="px-8 sm:px-12 py-3 sm:py-4 bg-accent text-white font-bold rounded-2xl hover:scale-105 transition shadow-lg shadow-accent/30 focus-visible:ring-2 focus-visible:ring-accent">BẮT ĐẦU</button>
        </div>
        ...
        <div x-show="started && !finished">
          <div class="flex justify-between items-center mb-6 sm:mb-8 font-code text-[10px] font-bold uppercase tracking-widest text-gray-500">
            ...
            <span class="text-accent" x-text="'Điểm: ' + score" aria-live="polite"></span>
          </div>
        ...
        <div x-show="finished">
          ...
          <h3 class="text-2xl sm:text-3xl font-bold mb-4 uppercase">Hoàn thành!</h3>
          <p class="text-primary font-bold text-xl sm:text-2xl mb-6" x-text="'Kết quả: ' + score + '/' + questions.length"></p>
          <button type="button" @click="reset()" class="px-6 py-2 border border-gray-300 dark:border-white/20 rounded-xl text-xs font-bold hover:bg-gray-100 dark:hover:bg-white/5 transition uppercase focus-visible:ring-2 focus-visible:ring-primary">Làm lại</button>
        </div>
      </div>
    </section>
  </main>
  ...
  <div x-data="academicToolkit()" class="fixed bottom-8 right-8 z-[150] flex flex-col items-end gap-3" aria-label="Academic toolbox">
    <div x-show="openTab === 'glossary'" x-transition x-cloak class="w-72 glass p-5 rounded-2xl shadow-2xl mb-2 overflow-hidden bg-white/90 dark:bg-surface/90 backdrop-blur-xl border border-white/20">
      ...
      <input type="text" x-model="searchQuery" placeholder="Tìm thuật ngữ..." aria-label="Tìm thuật ngữ"
      ...
      <div class="space-y-2 text-left">
        ...
        <template x-for="bm in bookmarks">
          <div class="flex justify-between items-center text-[10px] bg-gray-100 dark:bg-black/20 p-2 rounded border border-transparent dark:border-white/5">
            ...
            <button type="button" @click="removeBookmark(bm)" class="text-accent hover:text-red-600" aria-label="Xóa bookmark">✕</button>
          </div>
        </template>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-2">
      ...
      <button type="button" @click="addBookmark()" class="w-10 h-10 bg-white dark:bg-surface text-gray-600 dark:text-gray-300 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110 border border-gray-200 dark:border-white/10" aria-label="Lưu bài">
        <span class="text-sm">🔖</span>
      </button>
      <button type="button" @click="openTab = openTab ? '' : 'main'" class="w-14 h-14 bg-primary text-white rounded-full shadow-xl shadow-primary/30 flex items-center justify-center text-2xl transition-transform duration-300 hover:rotate-90 focus-visible:ring-2 focus-visible:ring-primary" aria-label="Mở công cụ">
    ...
    function mainApp(){
      return {
        ...
        infographics: [
          ...
          { title: "Hệ thống xã hội", chapter: "5", url: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1600&q=90" }
        ],
        ...
        speakChapter(idx) {
          ...
          if(!window.speechSynthesis) {
            showToast('Trình duyệt không hỗ trợ đọc.');
            return;
          }
    ...
    function quizApp(){
      return {
        ...
        hint() {
          ...
          showToast('Gợi ý: Đáp án có chữ cái ' + letter);
        },
        reset() {
        ...
        loadState() {
          ...
          if(s && s.started) {
            ...
            this.score = s.score;
            showToast('Tiếp tục tiến độ quiz trước đó.');
          }
        }
      }
    }
    ...
    function academicToolkit() {
      return {
        ...
        copyCitation() {
          ...
          navigator.clipboard.writeText(text).then(()=> showToast('Đã sao chép trích dẫn chuẩn học thuật!'));
        },
        addBookmark() {
          ...
          if (!this.bookmarks.includes(currentTitle)) {
            ...
            showToast('Đã lưu bài viết này!');
          } else {
            showToast('Bài viết đã được lưu trước đó.');
          }
